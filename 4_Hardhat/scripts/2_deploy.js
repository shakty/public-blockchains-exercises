...
async function main() {
  ...
  console.log(`Signer Address: ${await first.getAddress()}`);
  // return;
  ...
  const getContractManual = async (
    ...
    const abi = require(`../artifacts/contracts/${contractName}.sol/${contractName}.json`).abi;

    // d.2 Create the contract and print the address.
    const lock = new ethers.Contract(address, abi, signer);
  ...
  const readContract = async (lockContract = lock) => {
    ...
    const owner = await lock.owner(); // works now
    console.log(`Owner of Lock: ${owner}`);
    ...
    console.log(
      `UnlockTime: ${new Date(Number(unlockTime) * 1000).toLocaleTimeString()}`
    );
  };
